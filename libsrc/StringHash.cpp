...
void StringIntHash::CountLinesFromFile(const char * filename)
   {
   IFILE f = ifopen(filename, "rb");
   if (f == NULL) return;
   CountLinesFromFile(f);
   ifclose(f);
   }

void StringIntHash::CountLinesFromFile(FILE * f)
   {
   String buffer;

   while (!feof(f))
      IncrementCount(buffer.ReadLine(f).Trim());
   }

#ifdef __ZLIB_AVAILABLE__
void StringIntHash::CountLinesFromFile(IFILE & f)
   {
   String buffer;

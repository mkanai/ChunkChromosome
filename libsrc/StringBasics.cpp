...
String & String::ReadLine(FILE * f)
   ...
   // This funny construction is meant to avoid gcc warnings about ignoring the
   // return value of fscanf ...
   if (check[0] == '\n') if (fscanf(f, "%*1[\r]") == 0) ;
   if (check[0] == '\r') if (fscanf(f, "%*1[\n]") == 0) ;
...
int String::my_vsnprintf(char * buffer, int bufsize, const char * format, va_list args)
   ...
   int sanity = fread(buffer, 1, bufsize, my_vsnprintf_file);

   if (sanity != len)
      error("Internal sanity checks failed in my_vsnprintf\n");
